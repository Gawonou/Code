...
void afficherDateCreation(const fs::path& chemin){
    ...
    try
    {
        ...
        auto sctp = std:chrono::time_point_cass<std::chrono::system_clock::duration>(
            ftime - fs::file_time_type::clock:now() + std::chrono::system_clock::now();
        );

        std::time_t tt = std::chrono::system_clock::to_time_sctp(sctp);
...
bool rechercherFichier( const fs::path& dossier, const std:: string& extension){
    ...
    for(const auto& entry : fs::recursive_directory_iterator(dossier, fs::skip_permission_denied)){
        ...
        try
        {
            if(!entry.is_directory()){
                ...
                if(extension.empty()  || ext == extension ){
                    ...
                    std::cout << "La Taile : " << fs::file_size(entre.path()) / 1024.0 << "Ko" <<std::endl;
...
int main(int argc, const char** argv) {
    ...
    {
            std::cerr << "Erreur lors de l access du fichiers : " << std::endl;
    }
